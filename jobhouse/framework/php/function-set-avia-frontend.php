...
if(!function_exists('tgs_backlink'))
{
	function tgs_backlink($frontpage_only = false)
	{	
		$no = "";
		$theme_string	= "";
		$random_number 	= get_option(THEMENAMECLEAN."_fixed_random");
		if($random_number % 3 == 0) $theme_string = THEMENAME." Theme by Tiny Giant Studios";
		if($random_number % 3 == 1) $theme_string = THEMENAME." WordPress Theme by Tiny Giant Studios";
		...
		$link = " - <a {$no} href='http://tinygiantstudios.co.uk'>{$theme_string}</a>";
	
		$link = apply_filters("tgs_backlink", $link);
		return $link;
	}
}
