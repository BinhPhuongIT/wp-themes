...
if (!class_exists('avia_fb_likebox'))
{
	class avia_fb_likebox extends WP_Widget {
		...
		function form($instance) {
			...
			$instance = wp_parse_args( (array) $instance, array('url' => 'https://www.facebook.com/.at', 'height' => '258px', 'border' => 'yes') );
...
if (!class_exists('avia_partner_widget'))
{
	class avia_partner_widget extends WP_Widget {
		...
		function widget($args, $instance)
		{
			...
			if ( !empty( $title ) ) { echo $before_title . $title . $after_title; };
			echo '<a target="_blank" href="'.$ref_url.'" class="preloading_background  avia_partner1 link_list_item'.$addwidget.' '.$firsttitle.'" >'.$image_url.'</a>';
			if($this->add_cont == 2) echo '<a target="_blank" href="'.$ref_url2.'" class="preloading_background avia_partner2 link_list_item'.$addwidget.' '.$firsttitle.'" >'.$image_url2.'</a>';
			echo $after_widget;
